...
set INSTALL_DIR=%cd:\=/%/testinstalldll
...
cmake -G %VS_GENERATOR% -A %VS_ARCHITECTURE% -DBUILD_SHARED_LIBS=ON -DCMAKE_INSTALL_PREFIX=%INSTALL_DIR% ..\..
...
cmake -G %VS_GENERATOR% -A %VS_ARCHITECTURE% -DBUILD_SHARED_LIBS=ON -DCMAKE_INSTALL_PREFIX=%INSTALL_DIR% -Dprotobuf_ABSL_PROVIDER=package -DZLIB_ROOT=%INSTALL_DIR% -Dprotobuf_MSVC_STATIC_RUNTIME=OFF -Dprotobuf_BUILD_TESTS=OFF ..\..
cmake --build . --config Release --target install || goto :error
popd
